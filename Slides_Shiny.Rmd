...
- in teaching 
- making your methods accessible to other researchers (e.g.: [Curtailed RRT Made Easy](https://fabiolareiber.shinyapps.io/CurtailedRRT/))
...
Note that you have to use [tidyeval, non-standard evaluation](https://tidyeval.tidyverse.org/dplyr.html) in tidyverse functions:  `select(!!sym(input$selectVars))`
...
Open the `input_exercise_app.R` file. The file contains an almost finished shiny. It is your task, to fill in the missing inputs ! The app displays the average `Rain` or `Temperature` in Bangladesh  each year. 

1. Define a **file-upload input** with the ID `dataset`, where people can upload the dataset. We are using the [`1901_2019_Bangladesh_weather`] data in the `Data` folder.

2. Define an **slider input** ranging from 1901 to 2019 with the ID `year`, where the user can select the range of years to include in the plot. Tipp: supply a vector of length two, to make a "range"-slider (e.g., `value = c(40, 60)`)

3. Define a **drop-down input** with the ID `yaxis`, where the user can select the variable to plot on the y-axis `Temperature` or `Rain`.

4. Define a **radio-button input** with the ID `colors`, where the user can select one of the following `ggplot` coloring pallets: `c("Tomato1","#17B661","Skyblue1")` (or whatever you like).

5. Finally, create a **free-text input** with the ID `title`, where the user can specify the title of her plot.  
...
Your task is it to create an app which does an k-means clustering analysis on the `penguins` data (`Data\penguins.csv`)
...
You can load data (e.g., `read.csv("Data\penguins.csv")`) and create variables  which you can access in your entire app (for instance in the choice options  in selectInput), when you insert the code before the `ui`.
...
[shinyapps.io](https://www.shinyapps.io/?_ga=2.165307323.56038037.1588581253-1556918011.1538985794) is a platform for hosting Shiny web apps (applications). They also offer a free plan.
...
<!-- *********** NEW SLIDE ************** -->
---
## Shiny Addon packages

There are a number of additional shiny related packages, which add additional functionalities, input options, or designs to "base" shiny:
...
- `shinyjs`
- [`shinyalert`](https://github.com/daattali/shinyalert#input-modals)
...
- adds MathJax to the page and typesets the math expressions (if found) in the content
- need to be called only once in the app


Lets look at `example_withMathJax.R`.

<!-- *********** NEW SLIDE ************** -->
---
## `shinythemes`

... allows to set a different theme for your UI by using the theme argument inside `fluidPage()`
...
Try out different themes [here](https://shiny.rstudio.com/gallery/shiny-theme-selector.html). With the package ` thematic` you can even adjust your ggplots automatically, when you add:
...
Lets look at `example_shinythemes.R`
...
... extends widgets available in shiny. 

Lets look at `example_shinyWidgets.R`
...
... makes it easy to use Shiny to create dashboards. But the coolest think you get is `box()`


Lets look at `example_shinydashboard.R`
...
Either alone are in small groups:

- think about a Shiny application useful for your research or that you would find interesting (e.g., power analysis for the *t*-test, plotting different prior distributions, simulating data from a model and user-defined parameters, etc.)

- implement it ! 

- if you want, host the app on `shinyapps.io`


At the end we will look at all the cool apps and talk about difficulties/problems/questions encountered on the way.
